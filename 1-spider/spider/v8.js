...
function findALLURL($){
  ...
  $('#filter-options').find('a').each(function(i, node){//寻找 id为filter-options的面板下所有的a标签内的url
    ...
    hrefs.push(url);  // build url collection
  });
  hrefs = filterUrl(hrefs); // check url collections
  return hrefs; // return collection results
}
...
function filterUrl(arr){//过滤url，我们发现我们要的url都有 /xiaoqu/的结构 
  ...
  for(var i = 0; i < arr.length; i++){
    var url = arr[i];
    if(!url || url.indexOf('xiaoqu') === -1) continue; // check the url feature
    newArr.push(url);
  }
  return newArr;
}
